    ...
    try:
        ...
        for image in array_images:
            ...
            image_path = os.path.join(
                folder_path,
                f"page_{current_page}.jpg"
            )
            array_image_paths.append(
                image_path
            )
            image.save(
                image_path,
...
def grayscale_batch_processing(output_previous_function):
    ...
    with Pool(processes=number_processes) as pool:
        array_grayscale_images = pool.map(
            cv_utils.convert_grayscale,
            array_image_paths
        )
...
def increase_contrast_batch_processing(output_previous_function):
    ...
    with Pool(processes=number_processes) as pool:
        array_contrast_images_info = pool.map(
            cv_utils.increase_contrast,
            array_images_info
        )
...
def thresholding_batch_processing(output_previous_function):
    ...
    with Pool(processes=number_processes) as pool:
        array_binary_images_info = pool.map(
            thresholding,
            array_images_info
        )
...
def table_detection_batch_processing(output_previous_function):
    ...
    for image_info in array_images_info:
        ...
        image_page_rgb = cv2.cvtColor(
            image_page,
            cv2.COLOR_GRAY2RGB
        )

        layout = detectron2_model.detect(
            image_page_rgb
        )
        array_layout_tables = lp.Layout(
            [element for element in layout if element.type == 'Table']
        )
...
def ocr_tables_batch_processing(output_previous_function):
    ...
    tesseract_model = lp.TesseractAgent(
        languages='eng'
    )
...
def extract_pdf_text(input_pipeline):
    ...
    with multiprocessing.Pool(number_processes) as pool:
        array_pdf_pages_texts = pool.map(
            cv_utils.extract_page_text,
            array_page_info
        )
...
def document_fields_generation_batch_processing(output_previous_function):
    ...
    with Pool(processes=number_processes) as pool:
        array_json_array_document_fields = pool.map(
            ai_utils.generate_document_fields,
            array_pdf_pages_texts
        )
...
def sbe_fields_generation_batch_processing(output_previous_function):
    ...
    with Pool(number_processes) as pool:
        ...
        logging.info(f"ARRAY SBE FIELDS END GENERATE COMPLETED SBE FIELDS FUNCTION: {array_json_array_sbe_fields}")
...
def repeating_groups_generation_batch_processing(output_previous_function):
    ...
    json_array_full_sbe_fields = utils.merge_unique_sbe_fields_json_arrays(
        array_json_array_sbe_fields
    )
